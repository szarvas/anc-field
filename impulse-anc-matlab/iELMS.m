classdef iELMS < handle
    ...
    methods
        function object = iELMS(primary_order, secondary_order, inverse_delay, fs)
        ...
        function AddReference(this, noise_to_ref, act_to_ref)
            ix_1 = size(this.h_noise_to_reference_, 1) + 1;           
            ix_3 = 1;     
            
            for k = 1:size(noise_to_ref, 1)
                this.h_noise_to_reference_(ix_1, :, ix_3) = noise_to_ref(k,:);
                ix_3 = ix_3 + 1;
            end
        ...
        function Simulate(this, simulation_steps)
            ...
            noise = zeros(size(this.h_noise_to_error_,1), simulation_steps);
            
            for k = 1:size(this.h_noise_to_error_,1)
                noise(k,:) = bandnoise(1000, 1.0, this.fs_, simulation_steps);
                if k == 1
                    this.noise_at_error_ = filter(this.h_noise_to_error_(k,:), 1, noise(k,:));
                else
                    this.noise_at_error_ = this.noise_at_error_ + filter(this.h_noise_to_error_(k,:), 1, noise(k,:));
                end
            end
            ...
            for ix_reference = 1:size(this.h_noise_to_reference_, 1)
                for ix_noise = 1:size(this.h_noise_to_reference_, 3)
                    if ix_noise == 1
                        noise_at_reference(ix_reference,:) = filter(this.h_noise_to_reference_(ix_reference,:,ix_noise), 1, noise(ix_noise,:));
                    else
                        noise_at_reference(ix_reference,:) = noise_at_reference(ix_reference,:) ...
                           + filter(this.h_noise_to_reference_(ix_reference,:,ix_noise), 1, noise(ix_noise,:));
                    end
                end
            end
...
%                 end
                ...
                for ix_reference = 1:size(this.h_noise_to_reference_, 1)
                    ...
                    this.w_(ix_reference, :) = this.w_(ix_reference, :) + mu * fe * this.xbuf_(ix_reference, this.inverse_delay_+1:end);
                end
