...
#define	size_b_0	4
#define	size_b_1	5
#define	size_buf_in_0	12
#define	size_buf_in_1	1
#define	size_buf_out_0	12
#define	size_buf_out_1	1
...
__kernel void lrs_3(
	...
	const unsigned ix_mat_2	= (unsigned)pos[ix_pos(5,id)];

	const unsigned size_f	= size_a_0 / 2;

	const float a0_0 = a[ix_a(0,ix_mat_0)];
	const float a0_1 = a[ix_a(0,ix_mat_1)];
	const float a0_2 = a[ix_a(0,ix_mat_2)];
	const float b0_0 = b[ix_b(0,ix_mat_0)];
	const float b0_1 = b[ix_b(0,ix_mat_1)];
	const float b0_2 = b[ix_b(0,ix_mat_2)];
	...
	for (unsigned i = 1; i < size_a_0; ++i) {
		...
		gn_2 += b[ix_b(i,ix_mat_2)] * buf_in[ix_buf_in((ix_buf+i)%size_f+2*size_f,id)]
				- a[ix_a(i,ix_mat_2)] * buf_out[ix_buf_out((ix_buf+i)%size_f+2*size_f,id)];
	}
	...
	p_p1[ix_p_p1(x,y,z)] =
		...
		(l2 * (2.0f*p[ix_p(x-1,y,z)] + 2.0f*p[ix_p(x,y-1,z)] + 2.0f*p[ix_p(x,y,z-1)])
	...
	buf_out[ix_buf_out(ix_buf,id)] = 1/a0_0*(b0_0*xn_0 + gn_0);
	...
	const float xn_2 = a0_2/(l*b0_2) * (p_p1[ix_p_p1(x,y,z)] - p_d1[ix_p_d1(x,y,z)]) - gn_2/b0_2;
	buf_in[ix_buf_in(ix_buf+2*size_f,id)] = xn_2;
	buf_out[ix_buf_out(ix_buf+2*size_f,id)] = 1/a0_2*(b0_2*xn_2 + gn_2);
}
